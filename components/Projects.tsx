...
import { 
  ...
  Tag, ChevronRight as ChevronRightIcon, Flag
} from 'lucide-react';
...
const STATUS_COLUMNS: { label: string; value: TaskStatus; color: string; dot: string }[] = [
  { label: 'Backlog', value: 'pending', color: 'slate', dot: 'bg-slate-400' },
  { label: 'In Progress', value: 'in-progress', color: 'blue', dot: 'bg-blue-500' },
  { label: 'Review', value: 'review', color: 'amber', dot: 'bg-amber-500' },
  { label: 'Completed', value: 'completed', color: 'emerald', dot: 'bg-emerald-500' }
];
...
const STATUS_THEME: Record<TaskStatus, string> = {
  pending: 'bg-slate-100/50 text-slate-500 border-slate-200',
  'in-progress': 'bg-blue-50/50 text-blue-600 border-blue-100',
  review: 'bg-amber-50/50 text-amber-600 border-amber-100',
...
export const TaskCard: React.FC<{ task: ProjectTask; onUpdate: (updates: Partial<ProjectTask>) => void; onDelete: () => void }> = ({ task, onUpdate, onDelete }) => {
  ...
  return (
    <div 
      className={`p-6 bg-white border border-slate-100 rounded-[2rem] shadow-sm hover:shadow-float transition-all group relative cursor-pointer select-none ${task.status === 'completed' ? 'opacity-60 grayscale bg-slate-50/50' : ''}`}
    ...
    >
      <div className="flex justify-between items-start mb-4">
        <div className="flex gap-2">
           <span className={`px-2.5 py-1 rounded-lg text-[8px] font-bold uppercase tracking-widest border flex items-center gap-1.5 ${PRIORITY_THEME[task.priority]}`}>
             <Flag size={8} />
             {task.priority}
           </span>
           <span className={`px-2.5 py-1 rounded-lg text-[8px] font-bold uppercase tracking-widest border ${STATUS_THEME[task.status]}`}>
             {task.status.replace('-', ' ')}
           </span>
        </div>
        <div className="flex gap-2">
      ...
      <h4 className={`text-sm font-bold text-slate-900 leading-snug mb-3 ${task.status === 'completed' ? 'line-through text-slate-400' : ''}`}>
        {task.title}
      </h4>
        ...
        <div key={col.value} className="space-y-6">
          <div className="flex items-center justify-between px-4">
             <div className="flex items-center gap-3">
                <div className={`w-2 h-2 rounded-full shadow-lg ${col.dot}`}></div>
                 ...
                 <div className="flex items-center gap-8 flex-1">
                    ...
                    <div>
                       ...
                       <div className="flex items-center gap-3 mt-1">
                          <span className={`text-[8px] font-bold uppercase tracking-widest px-2.5 py-0.5 rounded-lg border flex items-center gap-1.5 ${PRIORITY_THEME[task.priority]}`}><Flag size={8}/>{task.priority}</span>
                          <span className={`text-[8px] font-bold uppercase tracking-widest px-2.5 py-0.5 rounded-lg border ${STATUS_THEME[task.status]}`}>{task.status.replace('-', ' ')}</span>
                          <span className="text-[8px] font-bold text-slate-300 uppercase tracking-[0.2em] ml-2">{task.category}</span>
                       </div>
                    </div>
                 </div>
                 <div className="flex items-center gap-12">
                    <span className="text-[10px] font-bold text-slate-300 uppercase tracking-widest">{task.dueDate ? new Date(task.dueDate).toLocaleDateString() : 'N/A'}</span>
